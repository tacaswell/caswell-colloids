...
def run_steps(start,max_accepted_steps,max_steps,generate_step,log_p_fun):
...
def lj_generator(ep,rmin):
    '''
    Generates Lenard-Jones penitential functions for use in p_fun_generator

    ep is the over all energy scale, rmin is the location of the minimum
    
    '''
    def lj(r):
        r_tmp = np.power(rmin**2/r,3)
        return ep*(np.power(r_tmp,2) - 2 * r_tmp)
...
def list_loop(p_list,n,lj):
    for j in range(0,n):
        ...
        prop_point = prop_point/np.sqrt(np.sum(prop_point**2))*pow(len(p_list)/4,1.0/3)
        chain = run_steps(prop_point,5000,500000,gen_step,lpf)
